...
import { FETCH_DETAIL_WORK_TIMES_REQUESTED } from './work_time_is_real.action';

@Component({
  selector: 'app-list',
  templateUrl: './work_time_is_real.component.html',
  styleUrls: ['./work_time_is_real.component.scss']
})
export class WorkTimeIsRealComponent extends BaseComponent implements OnInit {
  public reducer: String = 'Admin.User.work_time_is_real';
  ...
  constructor(private route: Router, activeRoute: ActivatedRoute) {
    ...
    this.navigationSubscription = this.route.events.subscribe((e: any) => {
      if (e instanceof NavigationEnd) {
        this.work_times = [JSON.parse(sessionStorage.getItem('detail'))];
        console.log('haha', this.work_times);
